...
func BFS(g *Graph, startVertex *Vertex, visitCb func(string)) {
...
func BFSRoute(g *Graph, startVertex *Vertex, endVertex *Vertex, visitCb func(string)) {
	// initialize queue and visited vertices map
	vertexQueue := &queue{}
	visitedVertices := map[string]bool{}
	...
	for {
		...
		if currentVertex.Key == endVertex.Key {
			break
		}
